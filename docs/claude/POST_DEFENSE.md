...
|---|---|---|---|
| 2026-09-25 | **1** | The methods define the health indicator as **any record** describing health data. `health_breach` (`156:144`) instead reads ONE `information_affected` per event: Stage 3 (`152:93`) keeps the single source record with the longest string, and Gate 2 (`153:62-71`) does not re-aggregate it across chained components. **5 of 405** analysis events mention health in a source record but carry 0 - treated: DISH Network 2023-02-22 (chain CH-22), Altice USA 2024-02-29 (CH-32); control: ABM Industries 2018-01-08, Wabtec 2022-03-15, Okta 2023-09-23 (all three lost inside their own firm-day, Wabtec by 9 characters). A sixth, IMA Financial 2022-10-18 (control, CH-31), is in the 489 but not the 405 and is corrected too. The error is one-directional: all 24 currently-flagged events survive an `"Affected":"Yes"`-only reading, so the indicator under-counts and never over-counts. The treated health cell is empty (0 of 109) for this reason alone. Fixed in `scripts/214` as ledger entries; v3 left frozen. | `0a4ad57` (log), see following commit (change). **Constants moved** (51 of 94, all F1 and F4; `N`, `treated`, `control`, `parent_ciks`, `n_tests` and the classifier id unchanged): F1_30 coef .0032->.0040, CV3 p .9252->.9061, WCR p .9113->.8889, MDE80 .0960->.0944; F1_90 coef -.0390->-.0410, CV3 p .6263->.6053, WCR p .5395->.5136, MDE80 .2256->.2234; F1_180 coef .0227->.0233, CV3 p .8211->.8159, WCR p .7950->.7901, MDE80 .2829->.2815; F4 placebo coef -.0837->-.0862, CV3 p .3257->.3116, WCR p .2444->.2308. Primary-family min BH p .9252->.9061; placebo BH .3257->.3116; sensitivity-family min BH p .9942->.9864. **No verdict changed** - all eight rows of `239_verdict_table.csv` remain NULL. 30d leave-one-cluster-out sign flips 4->3 (Salesforce no longer flips; Sprint, Sirius XM and T-Mobile still do); 90d and 180d unchanged at 1 each. **Health counts**: 405 -> 26 health (2 treated / 24 control) and 379 non-health (107 treated / 272 control), from 21 health (0 treated / 21 control); 489 -> 30 health (2 treated / 28 control). **Re-run**: 214, **235**, 224, 227, 229, 228, 239. 235 is in the list because `health_breach` reaches 224 through `b_scope_events.csv`, which 235 rebuilds from the loader (`235:226`); 228 and 239 because they read moved inputs. Constants were written by 227 itself, after retiring the superseded baseline so its assertion could not block the legitimate change (the old file stays in git at `cc9a8cc`). Gates: 217 64/64, 240 check 7/7, 210 PASS after staging. |
