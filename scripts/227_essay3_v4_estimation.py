...
================================================================================================
Sample: outputs/essay3_v4/e_analysis_sample.csv, in_analysis_sample == 1 (scripts/224). CANONICAL_V4 via scripts/236;
v3 constants untouched; results go to outputs/essay3_v4/constants_essay3_v4.json (assertion baseline: the first
...
Outputs: outputs/essay3_v4/f1_ladder.csv, f1_logit_ame.csv, f1_cluster_diagnostics.csv, f4_placebo.csv, f2_baseline.csv,
         f3_sensitivities.csv, f3_sic2_cells.csv, f3_loco.csv, f5_ceo.csv, f6_director.csv, i_tests.csv,
         constants_essay3_v4.json, 202_estimation.log

REBUILD V4 NOTE
---------------
Copied from scripts/202_essay3_q2_estimation.py. Only the data sources move; the method does not.
...
cp_ = OUT / 'constants_essay3_v4.json'
if cp_.exists():
    ...
    log('\nAssertion check vs constants_essay3_v4.json: PASS')
else:
    cp_.write_text(json.dumps(C, indent=1))
    log('\nBaseline constants_essay3_v4.json WRITTEN (later runs assert against it)')
(OUT / '227_estimation.log').write_text('\n'.join(L) + '\n', encoding='utf-8')
