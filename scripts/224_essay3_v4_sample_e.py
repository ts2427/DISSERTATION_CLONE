...
===================================================================================================
Runs on CANONICAL_V4 through scripts/236's loader; v3 is frozen and untouched.
Outcome = final classifier v2 (scripts/195, commit 6f7be7a) via outputs/essay3_v4/c2_outcomes_events.csv.
...
Outputs: outputs/essay3_v4/e1_cik_resolution.csv, e_ledger.csv, e_analysis_sample.csv, 199_sample.log

REBUILD V4 NOTE
---------------
Copied from scripts/199_essay3_q2_sample_e.py. Only the data sources move; the method does not.
...
spec = importlib.util.spec_from_file_location('clf2', 'scripts/220_essay3_v4_classifier_v2.py')
...
# v3's hard-coded outcome-CIK patch list lived here. v4 resolves outcome_cik by
# rule in scripts/234 and reconciles it against that list in scripts/233, so the
# dict is deliberately NOT carried over - re-applying it would correct the same
# event twice. scripts/233 currently reports every row 'agree'.
...
hdr('E1 — outcome CIK: resolved upstream, NOT re-applied here')
# v3 patched the outcome CIK here and re-fetched the corrected filer's documents
# mid-script. v4 resolves outcome_cik upstream (scripts/234) and reconciles it
# against v3's dict (scripts/233), so this section only REPORTS.
# require_resolve_settled() aborts unless every 233 row says 'agree', so this
# script cannot run against an unsettled reconciliation.
_agreed = V4.require_resolve_settled()
log(f'scripts/233: {len(_agreed)} reconciled row(s), all "agree" — RESOLVE is not re-applied')
res_rows = []
for _, e in sc.iterrows():
    _oc = e.get('outcome_cik')
    if pd.isna(_oc) or int(_oc) != int(e['final_cik']):
        res_rows.append(dict(final_cik=e['final_cik'], org_name=e['org_name'],
                             breach_date=e['breach_date'], reported_date=e['reported_date'],
                             treated=int(e['fcc_form499']),
                             action=('UNRESOLVED' if pd.isna(_oc) else 'RULE'),
                             outcome_cik=_oc, reason=f"scripts/234 rule: {e.get('outcome_rule', '')}"))
R1 = pd.DataFrame(res_rows, columns=['final_cik', 'org_name', 'breach_date',
                                     'reported_date', 'treated', 'action',
                                     'outcome_cik', 'reason'])
R1.to_csv(OUT / 'e1_cik_resolution.csv', index=False)
log(f'events whose outcome_cik differs from final_cik, or is unresolved: {len(R1)}')
log(R1.to_string(index=False) if len(R1) else '  (none)')
...
OUTC['outcome_cik'] = OUTC.set_index(KEY).index.map(
    sc.drop_duplicates(KEY).set_index(KEY)['outcome_cik'])
# (v3 patched individual events' outcomes here from `repl`; v4 has no patch to
#  apply - the corrected filer is already the outcome_cik used throughout.)
...
for _, r in OUTC.iterrows():
    oc = int(r['outcome_cik']) if pd.notna(r['outcome_cik']) else -1
    if pd.isna(r['outcome_cik']):
        # No candidate CIK filed an 8-K in the window (scripts/234), so the event
        # fails the outcome-data requirement. This is where v3 special-cased Nokia.
        has8k.append(0)
        continue
    if oc not in fcache:
...
chunks = []
for fp in ['Data/wrds_v4/crsp_dsf.csv',
           'Data/wrds_v4/crsp_dsf_topup_20260919T000910Z.csv',
           'Data/wrds_v4/crsp_dsf_topup_20260919T135055Z.csv',
           'Data/wrds_v4/crsp_dsf_topup_20260919T135055Z_issuer.csv']:
    if Path(fp).exists():
...
mk = pd.read_csv('Data/wrds_v4/crsp_dsi.csv', usecols=['date', 'vwretd'])
