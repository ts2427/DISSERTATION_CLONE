...
print("\nPrerequisites:")
print("  ✓ WRDS data merged (16_merge_wrds_data.py)")
print("  ✓ Audit Analytics merged (32_merge_audit_data.py)")
print("\nPress Enter to start, or Ctrl+C to cancel...")
input()

# Define essential enrichment scripts
pipelines = [
    {
        'number': 1,
...
for pipeline in pipelines:
    ...
    try:
        ...
        script_path = f"scripts/{pipeline['script']}"
        if not os.path.exists(script_path):
            print(f"\n⚠ WARNING: Script not found - {script_path}")
            print(f"  Skipping this enrichment...")
            results.append({
...
print(f"Failed/Skipped: {sum(not r['success'] for r in results)}")
...
for filename in enrichment_files:
    ...
    if os.path.exists(filepath):
        ...
        print(f"  ⊘ {filename:45} (NOT CREATED)")

print("\n" + "=" * 80)

successful_count = sum(r['success'] for r in results)

if successful_count == len(pipelines):
    print("✓✓✓ ALL ENRICHMENTS COMPLETE ✓✓✓")
    print("\nNext step: Run scripts/50_merge_all_enrichments.py")
    ...
    print(f"⚠ PARTIAL SUCCESS - {successful_count}/{len(pipelines)} enrichments completed")
    print("\nYou can still proceed with merge using successful enrichments")
    print("Run: python scripts/50_merge_all_enrichments.py")
else:
    print("✗ ALL ENRICHMENTS FAILED OR SKIPPED")
    print("\nTroubleshooting:")
    print("  1. Make sure enrichment scripts exist in scripts/ folder")
    print("  2. Check that base dataset exists:")
    print("     Data/processed/FINAL_DISSERTATION_DATASET_WITH_GOVERNANCE.xlsx")
