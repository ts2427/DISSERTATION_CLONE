"""
ESSAY 3 QUERY 2 — PART C: ITEM 5.02 DEPARTURE CLASSIFIER (NEW; deterministic rules, no ML)
==========================================================================================
...
DEP = re.compile(
    r'(?i)\b(resign(?:s|ed|ing|ation|ations)?|retire(?:s|d|ment|ing)?|step(?:s|ped|ping)?\s+down|'
    r'depart(?:s|ed|ing|ure)?|terminat(?:e|es|ed|ing|ion)\s+(?:of\s+)?(?:his|her|their|the)?\s*'
    r'(?:employment|service)|(?:his|her|their)\s+employment\s+(?:with\s+the\s+\w+\s+)?(?:will\s+)?'
    r'(?:be\s+)?(?:automatically\s+)?(?:terminat\w+|end\w*|cease\w*)|(?:was|were|has\s+been|have\s+been)\s+'
    r'terminated\s+(?:as|from|without|for|by)|will\s+leave|(?:has\s+)?left\s+the\s+(?:company|corporation|'
    ...
    r'executive\s+(?:team|leadership|officers?))')
STRONG = re.compile(
    ...
    rf'retire(?:ment|s|d)?\s+(?:on|effective|as\s+of)\s+(?:or\s+about\s+)?(?:{MONTHS}))')
...
PRIOR_REF = re.compile(r'(?i)^(?:previously|prior\s+to\s+this)\b|\bhad\s+(?:previously\s+)?(?:resigned|retired|'
                       r'departed|stepped\s+down|stated)|previously\s+held\s+by|vacanc(?:y|ies)\s+(?:created|'
                       r'resulting|caused)\s+(?:by|from)')
...
BIND = re.compile(r'(?i)^\s*(?:(?:his|her|their)\s+(?:positions?|roles?|duties)\s+)?(?:as|from)\s+(?:the\s+)?'
...
def role_near(s, pos_start, pos_end):
    ...
    spans = [sp for sp in title_spans(s)  # titles the person keeps are not the ones being left
             if not re.search(r'(?i)(remain\w*|continu\w*|retain\w*|keep\w*)\s+(?:to\s+serve\s+)?(?:as\s+)?'
...
def classify(sec):
    ...
    for i, s in enumerate(sents):
        ...
        skip = (((CONDITIONAL.search(sm) or BIO.search(sm)) and not strong) or bool(PRIOR_REF.search(sm))
                or (bool(HARD.search(sm)) and not EFF_DATE.search(sm)))
        found = False
        if not skip:
...
if __name__ == '__main__':
    ...
    F = pd.DataFrame(rows)
    F.to_csv(OUT / 'c_filing_codes.csv', index=False)
    pd.DataFrame(prow).to_csv(OUT / 'c_person_rows.csv', index=False)
    pd.DataFrame(secs).to_csv(OUT / 'c_sections.csv', index=False)
    ...
    for _, e in ev.iterrows():
        ...
        for anc, t0 in [('rd', e['rdt']), ('bd', e['bdt'])]:
            for w in (30, 90, 180):
                ...
                rec[f'exec_departure_{w}_{anc}'] = int(x['exec_departure'].sum() > 0)
                rec[f'ceo_departure_{w}_{anc}'] = int(x['ceo_departure'].sum() > 0)
                rec[f'director_departure_{w}_{anc}'] = int(x['director_departure'].sum() > 0)
            ...
            rec[f'days_to_first_exec_departure_{anc}'] = (x['fdt'].min() - t0).days if len(x) else np.nan
            pl = g[(g['fdt'] > t0 - timedelta(days=180)) & (g['fdt'] <= t0)]
            rec[f'placebo_exec_departure_{anc}'] = int(pl['exec_departure'].sum() > 0)
            bl = g[(g['fdt'] >= t0 - timedelta(days=730)) & (g['fdt'] <= t0 - timedelta(days=181))]
            rec[f'baseline_exec_departures_{anc}'] = int(bl['exec_departure'].sum())
            rec[f'baseline_exec_rate_py_{anc}'] = round(int(bl['exec_departure'].sum()) / (550 / 365), 4)
        out.append(rec)
    O = pd.DataFrame(out)
    O.to_csv(OUT / 'c_outcomes_events.csv', index=False)
    ...
    for samp, d in [('scope 341', O2), ('Q1 Essay 3 sample 340', O2[O2['immediate_disclosure'].notna()])]:
        for anc in ('rd', 'bd'):
            for w in (30, 90, 180):
                ...
                cw.append(row)
    CW = pd.DataFrame(cw)
    CW.to_csv(OUT / 'c_crosswalk.csv', index=False)
    ...
    (OUT / '188_classifier.log').write_text('\n'.join(L) + '\n', encoding='utf-8')
