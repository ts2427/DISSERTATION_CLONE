"""
GOVERNANCE QUALITY HETEROGENEITY ANALYSIS - ESSAY 2 VOLATILITY VERSION
...
experience larger FCC-driven volatility increases because markets doubt the quality of
forced disclosure under weak governance.

Mechanism: FCC mandates fast disclosure -> breached firms with weak governance
cannot produce credible disclosures -> markets increase uncertainty for information quality risk
-> larger FCC volatility effect for governance-weak firms.
...
print("=" * 90)
print("GOVERNANCE QUALITY HETEROGENEITY ANALYSIS - ESSAY 2 VOLATILITY VERSION")
print("=" * 90)
...
print("\n  VOLATILITY by governance quality:")
gov_groups = df[df['volatility_change'].notna()].groupby('weak_governance')['volatility_change'].describe().round(4)
print(gov_groups)

print("\n  VOLATILITY by FCC and governance quality:")
fcc_gov_groups = df[df['volatility_change'].notna()].groupby(
...
non_fcc_weak = df[(df['fcc_reportable']==0) & (df['weak_governance']==1) & (df['volatility_change'].notna())]['volatility_change'].mean()
fcc_weak = df[(df['fcc_reportable']==1) & (df['weak_governance']==1) & (df['volatility_change'].notna())]['volatility_change'].mean()
non_fcc_strong = df[(df['fcc_reportable']==0) & (df['weak_governance']==0) & (df['volatility_change'].notna())]['volatility_change'].mean()
fcc_strong = df[(df['fcc_reportable']==1) & (df['weak_governance']==0) & (df['volatility_change'].notna())]['volatility_change'].mean()

print(f"    Non-FCC + Weak governance: {non_fcc_weak:.4f}pp")
print(f"    FCC + Weak governance: {fcc_weak:.4f}pp")
print(f"    FCC effect for weak-gov firms: {fcc_weak - non_fcc_weak:.4f}pp (DIFFERENCE)")
print(f"\n    Non-FCC + Strong governance: {non_fcc_strong:.4f}pp")
print(f"    FCC + Strong governance: {fcc_strong:.4f}pp")
print(f"    FCC effect for strong-gov firms: {fcc_strong - non_fcc_strong:.4f}pp (DIFFERENCE)")
...
# Volatility is already in percentage point form (ranges from -8.2 to +18.4)
...
print("\n  MODEL 1: Baseline FCC Effect (Essay 2)")
model1 = ols('volatility_outcome ~ fcc + health + financial + prior_breaches + firm_size_log + leverage + roa',
...
model2 = ols('volatility_outcome ~ fcc + weak_gov + health + financial + prior_breaches + firm_size_log + leverage + roa',
             data=reg_data).fit(cov_type='HC3')
...
print(f"    Weak Governance Effect: {coef_gov:.4f}pp (p={pval_gov:.4f})")
print(f"    R-squared = {model2.rsquared:.4f}")

# Model 3: FCC x Governance Weakness Interaction (KEY TEST)
print("\n  MODEL 3: FCC x GOVERNANCE WEAKNESS INTERACTION (Key Test)")
model3 = ols('volatility_outcome ~ fcc * weak_gov + health + financial + prior_breaches + firm_size_log + leverage + roa',
             data=reg_data).fit(cov_type='HC3')
...
print(f"    Weak Gov Main Effect: {coef_gov_m3:.4f}pp (p={pval_gov_m3:.4f})")
print(f"    FCC x Weak Governance: {coef_interact:.4f}pp (p={pval_interact:.4f})")
print(f"    R-squared = {model3.rsquared:.4f}")

if coef_interact != 0:
    print(f"\n    INTERPRETATION:")
    print(f"    - FCC effect for strong-gov firms: {coef_fcc_m3:.4f}pp")
    print(f"    - FCC effect for weak-gov firms: {coef_fcc_m3 + coef_interact:.4f}pp")
    print(f"    - Difference (interaction): {coef_interact:.4f}pp")

    if coef_interact > 0.1 and pval_interact < 0.05:
        print(f"\n    FINDING: FCC VOLATILITY INCREASE IS AMPLIFIED for weak-governance firms")
        print(f"    This supports the GOVERNANCE WEAKNESS MECHANISM")
    elif coef_interact < -0.1 and pval_interact < 0.05:
        print(f"\n    FINDING: FCC VOLATILITY INCREASE IS REDUCED for weak-governance firms")
        print(f"    Alternative interpretation: weak-gov firms disclose more completely")
    else:
        print(f"\n    FINDING: NO SIGNIFICANT INTERACTION")
        print(f"    Governance quality does not moderate FCC volatility effect")
...
results_table.to_csv('outputs/tables/TABLE_GOVERNANCE_QUALITY_VOLATILITY_RESULTS.csv', index=False)
print(f"  [OK] Saved regression results: outputs/tables/TABLE_GOVERNANCE_QUALITY_VOLATILITY_RESULTS.csv")

print("\n" + "=" * 90)
print("ESSAY 2 VOLATILITY: GOVERNANCE QUALITY HETEROGENEITY ANALYSIS COMPLETE")
print("=" * 90)

print("\nKey Findings:")
print(f"  - Baseline FCC volatility effect: {coef_fcc_m1:.4f}pp (consistent with Essay 2)")
print(f"  - Governance weakness main effect: {coef_gov:.4f}pp")
print(f"  - FCC x Governance interaction: {coef_interact:.4f}pp (p={pval_interact:.4f})")

print("\nInterpretation:")
if coef_interact > 0.1 and pval_interact < 0.05:
    print("  MECHANISM CONFIRMED: Governance weakness amplifies FCC volatility increase")
    print("  Market mechanism: forced disclosure from weak-governance firms")
    print("  creates additional information uncertainty")
else:
    print("  Alternative mechanism: FCC volatility effect operates independently of governance")
    print("  FCC timing may increase uncertainty regardless of firm governance quality")

print("\n[OK] Essay 2 Mechanism 3 (Governance) analysis complete!")
