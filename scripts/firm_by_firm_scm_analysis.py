...
def run_python_scm():
    """Execute the Python SCM implementation to generate SCM results"""
    print("\n" + "="*70)
    print("RUNNING FIRM-BY-FIRM SCM (PYTHON)")
    print("="*70)
    ...
    try:
        ...
        if result.returncode == 0:
            print("[OK] Python SCM completed successfully")
            print(result.stdout)
        else:
            print("[ERROR] Python SCM failed:")
            print(result.stderr)
            return False
        ...
        print("[WARNING] SCM script not found: firm_by_firm_scm_python.py")
        return False
    except subprocess.TimeoutExpired:
        print("[WARNING] Python SCM timed out (>1 hour)")
        return False
    except Exception as e:
        print(f"[ERROR] Error running Python SCM: {e}")
        return False
