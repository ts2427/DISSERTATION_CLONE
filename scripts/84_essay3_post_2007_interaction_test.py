...
- FCC coefficient should be significant in 2007+ (post-regulation)
...
model_cols = ['volatility_change', 'fcc_reportable', 'immediate_disclosure', 'firm_size_log',
              'leverage', 'roa', 'post_2007', 'fcc_post_2007', 'return_volatility_pre']
...
X1 = sm.add_constant(reg_df[['fcc_reportable', 'immediate_disclosure', 'firm_size_log',
                              'leverage', 'roa', 'return_volatility_pre']].astype(float))
model1 = sm.OLS(reg_df['volatility_change'].astype(float), X1).fit(cov_type='HC3')
...
if len(reg_df_pre) > 10:  # Only run if enough observations
    X2 = sm.add_constant(reg_df_pre[['fcc_reportable', 'immediate_disclosure', 'firm_size_log',
                                      'leverage', 'roa', 'return_volatility_pre']].astype(float))
    model2 = sm.OLS(reg_df_pre['volatility_change'].astype(float), X2).fit(cov_type='HC3')
...
X3 = sm.add_constant(reg_df_post[['fcc_reportable', 'immediate_disclosure', 'firm_size_log',
                                   'leverage', 'roa', 'return_volatility_pre']].astype(float))
model3 = sm.OLS(reg_df_post['volatility_change'].astype(float), X3).fit(cov_type='HC3')
...
X4 = sm.add_constant(reg_df[['fcc_reportable', 'post_2007', 'fcc_post_2007', 'immediate_disclosure',
                              'firm_size_log', 'leverage', 'roa', 'return_volatility_pre']].astype(float))
model4 = sm.OLS(reg_df['volatility_change'].astype(float), X4).fit(cov_type='HC3')
...
output_file = Path('outputs/tables/essay3/TABLE_B8_post_2007_interaction_volatility.txt')
output_file.parent.mkdir(parents=True, exist_ok=True)

with open(output_file, 'w', encoding='utf-8') as f:
    f.write("TABLE B8: POST-2007 INTERACTION TEST - VOLATILITY OUTCOMES\n")
    f.write("Dependent Variable: Volatility Change (Post-Breach Volatility - Pre-Breach Volatility)\n")
    f.write("Tests whether FCC effect emerges after 2007 regulation (supporting causal effect) or existed before (industry effect)\n")
    f.write("\n")
    ...
    f.write("\n")
    f.write("Notes: FCC regulation (47 CFR § 64.2011) became effective in 2007. If the FCC effect on volatility reflects\n")
    f.write("regulatory burden, the coefficient should be non-significant pre-2007 and significant post-2007. If it reflects\n")
    f.write("industry characteristics, the coefficient should be similar across both periods.\n")
    f.write("\n")
    f.write("Control Variables: immediate_disclosure, firm_size_log, leverage, roa, return_volatility_pre\n")
    f.write("Standard errors (HC3 heteroskedasticity-consistent).\n")
    f.write("Significance levels: * p<0.10, ** p<0.05, *** p<0.01\n")

print(f"\n[OK] TABLE B8 (Volatility) saved to: {output_file}")
...
if not np.isnan(fcc_coef_pre) and fcc_pval_pre > 0.05 and fcc_pval_post < 0.05:
    ...
    print(f"[OK] The volatility increase comes from the constraint, not industry characteristics")
    ...
    print(f"[NOTE] Results show FCC effect in post-2007 period p={fcc_pval_post:.4f}")
