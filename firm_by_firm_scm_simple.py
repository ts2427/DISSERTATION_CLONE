"""
SIMPLIFIED FIRM-BY-FIRM SCM - PURE PYTHON
==========================================

Simpler, more robust implementation that works with breach-level data directly.
"""
...
print("\n" + "="*70)
print("FIRM-BY-FIRM SYNTHETIC CONTROL METHOD - PURE PYTHON")
print("="*70)
...
print("\n[2/3] Running SCM for each FCC firm...")
...
for i, treatment_id in enumerate(fcc_firms):
    ...
    try:
        ...
        # (many firms only breached after 2007, so we use first available year as pseudo-baseline)
        if len(post_idx) < 2:
            continue

        # If no pre-treatment data, use first year as baseline
        if len(pre_idx) == 0:
            pre_idx = [0]  # Use first year as reference

        # Compute gaps (simple: difference from control mean)
        gaps = y1 - y0
...
if len(results_df) > 0:
    ...
    print(f"[OK] Next: Run statistical inference and visualization")
else:
