...
def run_r_script():
    ...
    try:
        ...
        print("⚠ R script timed out (>1 hour)")
        return False
...
def load_scm_results():
    ...
    if not firm_summary_file.exists():
        print(f"✗ Results file not found: {firm_summary_file}")
    ...
    print(f"✓ Loaded results for {len(firm_results)} FCC firms")
    print(f"✓ Total gap observations: {len(all_gaps)}")
